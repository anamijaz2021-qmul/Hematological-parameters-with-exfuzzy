# -*- coding: utf-8 -*-
"""EDA_Final File_Updated on 25_6_2026

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1aMFMWH8G6CpC5i2RfI2KAUTHTnv7fip3

# 🩸 Hematological Parameters — Train/Test Validated Pipeline (v2)
**307 patients · IHD cohort · Split-validated unsupervised ML**

---

### What is different in this version
| Old approach | New approach (this notebook) |
|---|---|
| KNN imputation for all missing values | **Skewness-aware imputation**: median for skewed vars (\|skew\|>1), mean for symmetric vars |
| Clustering fit on 100% of data | **70:30 split** — cluster on train only, assign test to nearest cluster |
| No generalisation check | **Test-set silhouette** reported as validation metric |
| RFE/ExFuzzy trained on all data | RFE, Decision Tree, ExFuzzy all trained on train (70%), evaluated on test (30%) |

### Pipeline
| Phase | Section | Description |
|---|---|---|
| 1 | 1–3 | Environment, Loading, Cleaning (drop >40% missing, skew-aware imputation) |
| 2 | 4–11 | Descriptive & Inferential EDA (unchanged from before) |
| 3 | 12 | Min-Max Normalisation |
| 4 | 13 | **70:30 Train/Test Split** |
| 5 | 14 | Five clustering models on TRAIN, best selected by silhouette |
| 6 | 15 | Assign TEST set to nearest cluster — validate generalisation |
| 7 | 16 | Random Forest — feature importance ranking |
| 8 | 17 | Decision Tree — human-readable rules |
| 9 | 18 | ExFuzzy — fuzzy linguistic rules (Low/Medium/High) |
...
**Logic for each step:**
1. **Drop identifier columns** — Name, MR#, Code carry no analytical value and would corrupt distance-based ML (clustering, KNN) if accidentally included.
2. **Drop columns with >40% missing** — Imputing a variable that is mostly empty means manufacturing most of its values rather than estimating them. 40% is the threshold beyond which imputation becomes unreliable while still preserving all clinically important biomarkers (TG, LDL, HDL, WBC, HGB etc. are all <40% missing).
3. **Skewness-aware imputation** — Mean is the optimal estimator for symmetric distributions but is pulled by outliers in skewed distributions (e.g. Troponin-I, where a few critically ill patients have extreme values). Median is robust to this. We compute skewness on the *non-missing* values of each column: if \|skewness\| > 1, the distribution is meaningfully asymmetric → use median. Otherwise → use mean.
4. **Winsorise extreme outliers (5×IQR)** — Catches likely data-entry errors (e.g. Potassium=158) without touching genuine biological extremes.
5. **Download cleaned Excel** — for your own records / supplementary material.

"""

# ── Step 1: Drop identifiers ─────────────────────────────────────────────────
ID_COLS = ['Name','MR.#','Code']
df = raw.drop(columns=[c for c in ID_COLS if c in raw.columns])

# ── Step 2: Drop high-missing columns (>40%) ─────────────────────────────────
MISS_THRESHOLD = 0.40
miss_pct = df.isnull().mean()
dropped_cols = miss_pct[miss_pct > MISS_THRESHOLD].index.tolist()
df = df.drop(columns=dropped_cols)

print(f'Columns dropped (>{MISS_THRESHOLD*100:.0f}% missing): {len(dropped_cols)}')
print(f'  → {dropped_cols}')
print(f'Remaining columns: {df.shape[1]}')

# ── Step 3: Separate categorical flags from numeric ───────────────────────────
CAT_COLS = [c for c in CATEGORICAL if c in df.columns]
NUM_COLS = [
    c for c in df.columns
    if c not in CAT_COLS + ['IHD']
    and pd.api.types.is_numeric_dtype(df[c])
]
print(f'\nNumeric columns for imputation: {len(NUM_COLS)}')

# ── Step 4: Skewness-aware imputation ─────────────────────────────────────────
SKEW_THRESHOLD = 1.0
imputation_log = []
...
for col in NUM_COLS:
    ...
    imputation_log.append({
        'Variable': col, 'Skewness': round(skew_val,3),
        'Method': method, 'Fill_Value': round(fill_value,3),
        'N_Missing': n_missing
    })

imputation_log_df = pd.DataFrame(imputation_log).sort_values(
    'Skewness', key=abs, ascending=False)
print(f'Imputation log — {len(imputation_log_df)} variables had missing values:\n')
print(imputation_log_df.to_string(index=False))

print(f'\nMedian used for {(imputation_log_df["Method"]=="median").sum()} '
      f'skewed variables (|skew|>{SKEW_THRESHOLD})')
print(f'Mean used for {(imputation_log_df["Method"]=="mean").sum()} '
      f'approximately symmetric variables')

remaining_miss = df_imputed[NUM_COLS].isnull().sum().sum()
print(f'\nMissing values after imputation: {remaining_miss}')
df_clean = df_imputed.copy()

# ── Step 5: Winsorise extreme outliers (5×IQR) ────────────────────────────────
# Logic: clips values beyond 5x the interquartile range from Q1/Q3.
# This is a conservative fence — it only catches extreme data-entry errors
# (e.g. impossible lab values) while preserving genuine biological extremes.
def winsorise(series, k=5):
...
print('Winsorisation applied (5×IQR).')
print(f'\nFinal clean dataset: {df_clean.shape[0]} rows × {df_clean.shape[1]} columns')

# ── Download cleaned Excel ────────────────────────────────────────────────────
cleaned_filename = 'Hematological_parameters_CLEANED_v2.xlsx'
df_clean.to_excel(cleaned_filename, index=False)
files.download(cleaned_filename)
print(f'✅  Cleaned file downloaded: {cleaned_filename}')

"""---
## Phase 2 — Descriptive & Inferential Analysis
*(Sections 4–11, unchanged methodology from before)*

### Section 4 — Missingness Analysis (Raw Data)
"""

# ── Missingness in RAW data (before cleaning) ────────────────────────────────
miss_pct_raw = raw.drop(columns=[c for c in ID_COLS if c in raw.columns]).isnull().mean().sort_values(ascending=False)*100

fig, ax = plt.subplots(figsize=(16,5))
colors = ['#EF5350' if v>=50 else '#FFA726' if v>=40 else '#FFF176' if v>=20 else '#66BB6A' for v in miss_pct_raw]
miss_pct_raw.plot(kind='bar', color=colors, ax=ax, edgecolor='white')
ax.axhline(40, color='red', linestyle='--', linewidth=1.5, label='40% drop threshold')
ax.axhline(20, color='orange', linestyle='--', linewidth=1.2, label='20% caution')
ax.set_ylabel('Missing (%)')
ax.set_title('Missingness by Variable — Raw Data', fontweight='bold')
ax.legend()
plt.xticks(rotation=60, ha='right', fontsize=8)
plt.tight_layout(); plt.show()

fig, axes = plt.subplots(1,2,figsize=(18,5))
msno.matrix(raw.drop(columns=[c for c in ID_COLS if c in raw.columns]), ax=axes[0], sparkline=False, fontsize=8)
axes[0].set_title('Missing Data Matrix', fontweight='bold')
msno.heatmap(raw.drop(columns=[c for c in ID_COLS if c in raw.columns]), ax=axes[1], fontsize=8)
axes[1].set_title('Missingness Correlation', fontweight='bold')
plt.tight_layout(); plt.show()
...
summary['imputation_method'] = summary.index.map(
    dict(zip(imputation_log_df['Variable'], imputation_log_df['Method']))
).fillna('none needed')
...
X_norm_full = scaler_mm.fit_transform(df_clean[NUM_COLS])
...
### Section 13 — Split Before Clustering

**Logic:** Unsupervised clustering has no target label, so a conventional stratified split is not possible. Instead, we perform a **random 70:30 split of patients**. Clustering models are fit **only on the 70% train set**. The 30% test set is held out completely during model fitting. This mirrors how a clustering solution would need to behave in practice — generalising to *new, unseen* patients rather than just describing the data it was built on. In Phase 6, the test set is assigned to whichever cluster centre it is closest to, and we report a separate test-set silhouette to check whether the cluster structure holds up out-of-sample.
"""

# ── Section 13: 70:30 Train/Test Split ──────────────────────────────────────
TRAIN_FRAC = 0.70
...
if 'DBSCAN' in best_key and -1 in TRAIN_CLUSTER_LABELS:
    ...
    knn_fill.fit(X_train[clean_mask], TRAIN_CLUSTER_LABELS[clean_mask])
    filled = TRAIN_CLUSTER_LABELS.copy()
    filled[noise_mask] = knn_fill.predict(X_train[noise_mask])
...
"""---
## Final Summary & Download All Outputs
"""

# ── Final pipeline summary ───────────────────────────────────────────────────
print('═'*65)
print('COMPLETE PIPELINE SUMMARY (v2 — Train/Test Validated)')
print('═'*65)
print(f'Dataset           : {df_clean.shape[0]} patients × {df_clean.shape[1]} variables')
print(f'Columns dropped   : {len(dropped_cols)} (>{MISS_THRESHOLD*100:.0f}% missing)')
print(f'Imputation        : Skewness-aware (median if |skew|>{SKEW_THRESHOLD}, else mean)')
print(f'  Median used for : {(imputation_log_df["Method"]=="median").sum()} variables')
print(f'  Mean used for   : {(imputation_log_df["Method"]=="mean").sum()} variables')
print(f'─'*65)
print(f'Train/Test split  : {len(train_idx)} / {len(test_idx)} patients (70:30)')
print(f'Best clustering   : {best_method} (k={best_k})')
print(f'  Train Silhouette: {best_sil_train:.4f}')
print(f'  Test  Silhouette: {test_sil:.4f}')
print(f'─'*65)
print(f'RFE features      : {len(NUM_COLS)} → {optimal_n}')
print(f'Random Forest     : Train={train_acc:.3f}  Test={test_acc:.3f}')
print(f'Decision Tree     : Train={dt_train_acc:.3f}  Test={dt_test_acc:.3f}')
print(f'ExFuzzy           : Train={fuzzy_train_acc:.3f}  Test={fuzzy_test_acc:.3f}')
print('═'*65)

final_imp.sort_values('Importance',ascending=False).to_excel(
    'RFE_selected_features_v2.xlsx', index=False)
imputation_log_df.to_excel('Imputation_log.xlsx', index=False)

output_files = [
    'Hematological_parameters_CLEANED_v2.xlsx',
    'Imputation_log.xlsx',
    'RFE_selected_features_v2.xlsx',
    'clustering_comparison.png',
    'train_test_clusters.png',
    'rf_importance_and_test.png',
    'decision_tree.png',
    'model_quality_summary.png'
]
